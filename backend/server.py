...
async def sql_to_pdf(file: UploadFile = File(...)):
    """Convert SQL source code file to PDF with only code content"""
    temp_file = None
    output_file = None
    ...
    try:
        ...
        # Read SQL source code
        with open(temp_file, 'r', encoding='utf-8') as f:
            sql_code = f.read()
        
        # Create PDF with only code content (no line numbers, no headings)
        output_file = UPLOAD_DIR / f"{uuid.uuid4()}_sql.pdf"
